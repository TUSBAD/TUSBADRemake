...
# 軽減不可ダメージ
  # 引数を設定
    ...
    execute as @s[predicate=area:biome/underworld] run execute store result storage score_damage: Argument.Damage float 0.05 run data get entity @s Health
    execute as @s[predicate=!area:biome/underworld] run execute store result storage score_damage: Argument.Damage float 0.2 run data get entity @s Health

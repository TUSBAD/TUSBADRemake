...
# 軽減不可ダメージ
  # 引数を設定
    ...
    execute if entity @s[predicate=!tusb_remake:biome/underworld] run execute store result storage score_damage: Argument.Damage double 0.2 run data get entity @s Health
    execute if entity @s[scores={HP=..1}] run data modify storage score_damage: Argument.Damage set value 100

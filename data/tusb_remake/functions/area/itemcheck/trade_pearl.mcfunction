...
execute if entity @a[gamemode=!creative,predicate=tusb_remake:area/tradeisland] run summon item ~ ~ ~ {Tags:["Tradecheck"],PickupDelay:0,Item:{id:"minecraft:ender_pearl",Count:1b}}
execute as @a[predicate=tusb_remake:area/tradeisland,scores={UseEnderPearl=1..}] at @s run tellraw @s [{"text":"このエリアでは","color":"#ec024f","bold":true},{"translate":"%1$sを使用できません！","with":[{"text":"エンダーパール","color":"#0bce1e","bold": true}],"color":"#ec024f","bold":true}]
...
execute in tusb_remake:trade run scoreboard players reset @a[scores={UseEnderPearl=1..}] UseEnderPearl
